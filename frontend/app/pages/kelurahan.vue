...
interface Kelurahan {
  ...
  nama: string;
  active: boolean;
  t_kecamatan: Kecamatan;
  id_t_kecamatan?: number;
}

const BASE_URL = "http://localhost:8000/api";
const ENDPOINT = "t-kelurahan";
...
const listData = ref<Kelurahan[]>([]);
const listDataKecamatan = ref<Kecamatan[]>([]);
const modal = ref<boolean>(false);
const formData = ref<Kelurahan>(DEFAULT_FORM_VALUE);
...
async function fetchKecamatan() {
  const { data } = await $fetch(`${BASE_URL}/t-kecamatan`);
  listDataKecamatan.value = data;
}
...
async function handleEdit(editedItem: Kelurahan) {
  modal.value = true;
  await fetchKecamatan();
...
async function handleDelete(item: Kelurahan) {
      ...
      <div class="container mx-auto max-w-4xl">
        <section class="bg-white shadow p-6">
          <div class="flex justify-between">
            <h2 class="text-lg font-semibold mb-4">Daftar Kelurahan</h2>
            <button
          ...
          <table class="table-auto w-full">
            <thead>
              <tr>
                ...
                <th class="p-4 border-b">Nama Kelurahan</th>
        ...
        <div class="flex p-2">
          ...
          >
            ...
            <div class="w-full">
              ...
              >
              <select
                v-model="formData.id_t_kecamatan"
                class="w-full px-4 py-2 text-black border"
                placeholder="Select Data"
              >
                <option
                  v-for="(kecamatan, index) in listDataKecamatan"
                  :key="index"
                  :value="kecamatan.id"
                >
                  {{ kecamatan.nama }}
