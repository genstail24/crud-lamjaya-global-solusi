...
const province = ref<Province>(DEFAULT_FORM_VALUE);
...
function handleEdit(editedItem: Province) {
  modal.value = true;
  province.value = { ...editedItem };
}

function handleSubmit() {
  if (province.value.id) return handleUpdate();
...
async function handleSave() {
  ...
  if (success) {
    await fetchData();
    modal.value = false;
    province.value = DEFAULT_FORM_VALUE;
...
async function handleUpdate() {
  const { success, message } = await $fetch(
    `${BASE_URL}/${ENDPOINT}/${province.value.id}`,
    {
      method: "PUT",
      body: buildPayload(),
    }
...
async function handleDelete(item: Province) {
      ...
      <div class="container mx-auto max-w-4xl">
        <section class="bg-white shadow p-6">
          <div class="flex justify-between">
            <h2 class="text-lg font-semibold mb-4">Daftar Provinsi</h2>
            <button
          ...
          <table class="table-auto w-full">
            <thead>
              <tr>
                ...
                <th class="p-4 border-b">Nama Provinsi</th>
                <th class="p-4 border-b">Active</th>
                <th class="p-4 border-b">Action</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(province, index) in provinces" :key="province.nama">
                <td class="p-4 border-b text-center">{{ index + 1 }}</td>
                <td class="p-4 border-b text-center">{{ province.kode }}</td>
                <td class="p-4 border-b text-center">{{ province.nama }}</td>
                <td class="p-4 border-b text-center">
                  <input disabled type="checkbox" :checked="province.active" />
                </td>
                <td class="p-4 border-b text-center">
                  <div class="space-x-4">
                    <button
                      @click="handleEdit(province)"
                      class="px-4 py-2 bg-blue-400 text-white rounded-sm cursor-pointer transition hover:bg-blue-100 hover:text-blue-400"
                    >
                      Edit
                    </button>
                    <button
                      @click="handleDelete(province)"
